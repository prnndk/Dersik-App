<x-guest-layout>
    <x-jet-authentication-card>
        <x-slot name="logo">
            Register account for Dersik SMASA 22
        </x-slot>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                <x-jet-label for="tempatlahir" value="{{ __('Birth Place') }}" />
                <select class=" border-gray-300 block mt-1 w-full focus:border-indigo-300" name="tempatlahir">
                  @foreach ($kota as $kab)
                  @if (old('tempatlahir') == $kab->id)
            ...
            <div class="mt-4">
                <x-jet-label for="kelas_id" value="{{ __('Class') }}" />
                <select class="border-gray-300 block mt-1 w-full focus:border-indigo-300" name="kelas_id">
